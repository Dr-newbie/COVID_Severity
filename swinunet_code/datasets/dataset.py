...
class MedicalImageWithQuantDataset(Dataset):
    def __init__(self, csv_file, img_size, image_transform=None):
    ...
    def __getitem__(self, idx):
        # NIfTI 이미지 로드
        img_path = f"../final/1st/dataset/{self.data.iloc[idx, 1]}/{self.data.iloc[idx, 2]}/{self.data.iloc[idx, 3]}/nii/{self.data.iloc[idx, 5]}" #이건 우리 Severity data. 이게 원본
